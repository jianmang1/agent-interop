...
def fit_change_point_model(data, pheno):
    try:
        ...
        with pm.Model() as model:
            ...
            cp = pm.Uniform('cp', lower=x_scaled.min(), upper=x_scaled.max())  # 变点位置
            sigma = pm.HalfNormal('sigma', sigma=np.std(y))             # 噪声
            ...
            # 定义观测模型
            pm.Normal('obs', mu=y_pred, sigma=sigma + 1e-6, observed=y)
            ...
            initvals = {
                ...
                'b1': 0,
                'b2': 0,
                'cp': (x_scaled.min() + x_scaled.max()) / 2,  # 初始变点位置为中点
                'sigma': np.std(y)
            }
