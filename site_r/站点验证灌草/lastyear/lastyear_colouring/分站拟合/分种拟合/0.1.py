...
def add_lastyear_los(df):
    ...
    df['lastyear_叶全部变色期'] = None

    # 按站点分组处理
    for site_id, group in df.groupby('植物种名'):
        # 将分组数据按年份排序
        ...
        for i, row in group.iterrows():
            ...
            if not previous_row.empty:
                ...
                df.at[i, 'lastyear_叶全部变色期'] = previous_row.iloc[0]['叶全部变色期']
...
# 保存更新后的数据
grass_mean.to_csv('F:\\站点\\中国地面\\草本数据_每年均值（种）.csv', index=False, encoding='gbk')
wood_mean.to_csv('F:\\站点\\中国地面\\灌本数据_每年均值（种）.csv', index=False, encoding='gbk')