...
# 读取数据
file_path = r'E:\Desktop\物候差异\遥感\裸地转草.csv'
...
for i, metric in enumerate(phenology_metrics):
    ...
    if data_3.empty or data_4.empty:
        ...
        axes[i].set_title(f"{metric.upper()} 对比", fontsize=16)
        continue
    ...
    x_tick_labels = ['草本', '裸地']  # 左草右灌

    # 定义颜色 - 草本在前（索引0），灌木在后（索引1）
    box_colors = ['#2C5C8A', 'gray']  # 草本深蓝色，灌木暖橙色
    whisker_caps_colors = ['#87CEEB', 'lightgray']  # 草本浅蓝色，灌木浅橙色
    ...
    if sig_text:
        axes[i].text(1.5, line_height,  # 星号位置稍微上移
                     sig_text, horizontalalignment='center', verticalalignment='bottom',
                     fontsize=18, fontweight='bold', color='red')

    axes[i].set_title(f"{metric.upper()} 对比", fontsize=16)
    axes[i].set_ylabel("DOY", fontsize=14)
    axes[i].set_xticklabels(x_tick_labels, fontsize=12)
...
# 保存图像
output_plot_path = r'E:\Desktop\物候差异\遥感\裸地转草.png'
