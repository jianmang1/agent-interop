...
def process_year(year):
    ...
    def process_data():
        ...
        valid_center_prev = (src_array == 4) & (src_prev_array == 7)
        ...
        for mean_path, prefix in mean_config:
            ...
            with rasterio.open(mean_path) as mean_src:
                ...
                for row, col in zip(valid_rows_final, valid_cols_final):
                    ...
                    n7_current = np.count_nonzero(window_current == 7)
                    n4_current = np.count_nonzero(window_current == 4)
                    si_current = n7_current / (n7_current + n4_current) if (n7_current + n4_current) > 0 else 0

                    window_prev = src_prev_array[min_row:max_row + 1, min_col:max_col + 1]
                    n7_prev = np.count_nonzero(window_prev == 7)
                    n4_prev = np.count_nonzero(window_prev == 4)
                    si_prev = n7_prev / (n7_prev + n4_prev) if (n7_prev + n4_prev) > 0 else 0
                    ...
                    sos7_mean, sos7_var = calc_stats(7)
                    ...
                    if key not in results:
                        results[key] = {
                            ...
                            f'{prefix}_7_mean': sos7_mean,
                            f'{prefix}_7_var': sos7_var,
...
if __name__ == "__main__":
    ...
    output_path = r'D:\MODIS43A4_2024\all_output3.csv'
