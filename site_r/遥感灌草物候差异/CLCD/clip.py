...
clcd_folder = r"F:\转变\1985-2022中国全域逐年地表覆盖产品(CLCD)"
output_folder = r"F:\转变\masked_CLCD"
...
for year in range(2001, 2023):
    clcd_file = os.path.join(clcd_folder, f"CLCD_v01_{year}.tif")
    if not os.path.isfile(clcd_file):
        print(f"文件不存在: {clcd_file}")
        continue

    # 打开当前年份的地表覆盖产品文件
    clcd_ds = gdal.Open(clcd_file)
    if clcd_ds is None:
        print(f"无法打开文件: {clcd_file}")
        continue
    ...
    # 创建临时重采样文件
    temp_resampled_file = os.path.join(output_folder, f"temp_resampled_CLCD_v01_{year}.tif")
    ...
    # 创建输出文件
    output_file = os.path.join(output_folder, f"masked_CLCD_v01_{year}.tif")
