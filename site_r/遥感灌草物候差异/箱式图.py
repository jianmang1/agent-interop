...
def load_data():
    # 读取Excel文件
    df = pd.read_excel('D:/MODIS43A4_2024/combined_output1.xlsx')
    ...
    for pheno in pheno_list:
        ...
        # 处理草本数据（分别处理_31, _32, _33）
        for subclass in [31, 32, 33]:
            grass = df[['row', 'col', f'{pheno}_{subclass}']].copy()
            grass['CLASS'] = '草本'
            grass['Subclass'] = f'grass_{subclass}'
            grass['物候期'] = pheno
            grass.rename(columns={f'{pheno}_{subclass}': 'Value'}, inplace=True)
...
def plot_difference_boxplot(df):
    ...
    hue_order = ['灌木', 'grass_31', 'grass_32', 'grass_33']
    ...
    # 添加显著性标记
    subclasses = ['grass_31', 'grass_32', 'grass_33']
    for pheno_idx, pheno in enumerate(df['物候期'].unique()):
        for subclass_idx, subclass in enumerate(subclasses, start=1):
            ...
            y_max = max(current_data.max(), shrub_data.max()) + 3
