...
# 定义文件路径和配置    (r'D:\MODIS43A4_Value\POS\masked2_masked_all_PoS_value_{}.tif', 'POS'),
#     (r'D:\MODIS43A4_Value\POS\masked2_masked_all_PoS_value_{}.tif', 'ROG'),
#     (r'D:\MODIS43A4_Value\POS\masked2_masked_all_PoS_value_{}.tif', 'ROS'),
mean_config = [
    (r'D:\MODIS43A4_value\SOS\masked2_masked_all_SoS_value_{}_masked_tibet.tif', 'SOS'),
    (r'D:\MODIS43A4_Value\POS\masked2_masked_all_PoS_value_{}_masked_tibet.tif', 'POS'),
    (r'D:\MODIS43A4_value\EOS\masked2_masked_all_EoS_value_{}_masked_tibet.tif', 'EOS'),
]
...
def process_files(config):
    directory, label = config
    files_direct = [directory.format(year) for year in years if os.path.exists(directory.format(year))]

    print(f"Processing {label} files: {files_direct}")

    # 检查文件数量是否正确
    if len(files_direct) != len(years):
        raise ValueError(f"{label} 文件数量与年份数量不匹配")
    ...
    std_values_template = np.zeros_like(mean_values_template)
    ...
    for window in windows:
        ...
        std_values = np.zeros_like(data_array[0], dtype=np.float32)
        ...
        for i in range(data_array.shape[1]):
            for j in range(data_array.shape[2]):
                ...
                if len(valid_pixel_values) > 0:
                    ...
                    std_values[i, j] = np.sqrt(np.mean((valid_pixel_values - mean_values[i, j]) ** 2))
                else:
                    mean_values[i, j] = np.nan
                    std_values[i, j] = np.nan

        mean_values = np.where(np.isnan(mean_values), -9999.0, mean_values).astype(np.float32)
        std_values = np.where(np.isnan(std_values), -9999.0, std_values).astype(np.float32)
        ...
        std_values_template[window.row_off:window.row_off + window.height,
        window.col_off:window.col_off + window.width] = std_values
    ...
    with rasterio.open(os.path.join(output_dir, 'mean_values.tif'), 'w', **profile) as dst:
        dst.write(mean_values_template, 1)

    with rasterio.open(os.path.join(output_dir, 'std_values.tif'), 'w', **profile) as dst:
        dst.write(std_values_template, 1)

    with rasterio.open(os.path.join(output_dir, 'slope_values.tif'), 'w', **profile) as dst:
        dst.write(slope_values_template, 1)

    with rasterio.open(os.path.join(output_dir, 'p_values.tif'), 'w', **profile) as dst:
        dst.write(p_values_template, 1)


# 处理所有配置项
for config in mean_config:
