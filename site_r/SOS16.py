...
for batch in range(total_batches):
    ...
    for global_plot_index in range(start_index, end_index):
        ...
        try:
            ...
            for method in meth:  # 按预定义顺序匹配
                # 使用正则表达式或更精确的模式匹配
                if f"_{method}_" in threshold or threshold.startswith(method) or threshold.endswith(method):
    ...
    # --- 保存图像 ---
    output_filename = f'F:\\pythonforR\\sos3\\SOS_scatter_plots_withoutsjm_batch3_{prefix}.png'
...
# 保存汇总图
summary_output_filename = f'F:\\pythonforR\\sos3\\SOS_best_R_summary3.png'
