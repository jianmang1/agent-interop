...
# 定义配置项和TIFF文件路径
mean_config = [
    (r'D:\MODIS43A4_2024\SOS2\mean_values.tif', 'SOS'),
    (r'D:\MODIS43A4_2024\POS2\mean_values.tif', 'POS'),
    (r'D:\MODIS43A4_2024\EOS2\mean_values.tif', 'EOS'),
    (r'D:\MODIS43A4_Value\SOS\mean_values.tif', 'vSOS'),
    (r'D:\MODIS43A4_Value\POS\mean_values.tif', 'vPOS'),
    (r'D:\MODIS43A4_Value\EOS\mean_values.tif', 'vEOS'),
    (r'D:\MODIS43A4_Value\AOS\mean_values.tif', 'AOS'),
    (r'D:\MODIS43A4_2024\ROG\mean_values.tif', 'ROG'),  # 修正路径
    (r'D:\MODIS43A4_2024\ROS\mean_values.tif', 'ROS'),  # 修正路径
]
...
for tiff_path, config_name in mean_config:
    for mask_file in mask_files:
        if not os.path.exists(tiff_path):
            ...
            continue
        valid_data = extract_valid_data(tiff_path, mask_file)
        mean_val = np.mean(valid_data) if len(valid_data) > 0 else np.nan
        var_val = np.var(valid_data) if len(valid_data) > 0 else np.nan
        results.append({
            'Config': config_name,
...
for tiff_path, config_name in mean_config:
    if not os.path.exists(tiff_path):
        print(f"文件未找到: {tiff_path}. 跳过。")
        continue
    valid_data = extract_valid_data(tiff_path, mask_path=None)
    mean_val = np.mean(valid_data) if len(valid_data) > 0 else np.nan
    var_val = np.var(valid_data) if len(valid_data) > 0 else np.nan
    results.append({
        'Config': config_name,
...
# 保存到Excel文件
output_excel = r"D:\MODIS43A4_Value\mean_variance_results.xlsx"
...
# 主标题
fig.suptitle('Violin Plots of Valid Pixel Values (≥1) by Configuration and Mask',
             fontsize=16, fontweight='bold', y=0.98)

# 布局调整
plt.tight_layout(rect=[0, 0.05, 1, 0.95])
plt.savefig(r"D:\MODIS43A4_Value\mean_variance_violinplot_3x3.png", dpi=300, bbox_inches='tight')
plt.show()
