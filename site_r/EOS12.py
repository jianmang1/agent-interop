...
for batch in range(total_batches):
    ...
    for i in range(start_index, end_index):
        ...
        if not valid_data.empty:
            ...
            # 计算当前子图在批次内的行和列
            row_in_batch = plot_index // 6  # 每行5个子图
            col_in_batch = plot_index % 6  # 列号0-4
            letter = ['a', 'b', 'c', 'd', 'e', 'f'][row_in_batch]  # 每行对应一个字母
    ...
    # 添加大图的大标题
    fig.text(0.5, 0.06, 'Leaf colouring (DOY)', ha='center', fontsize=30, weight='bold')
    fig.text(0.04, 0.5, 'MOD43A4 EOS (DOY)', va='center', rotation='vertical', fontsize=30, weight='bold')
    ...
    # 保存当前批次的大图到文件
    plt.savefig(f'F:\pythonforR\EoS_yanzhen2\\EOS_scatter_plots_withoutsjm_batch1_{batch + 1}.png', dpi=600, bbox_inches='tight')
    plt.clf()
    plt.close(fig)