...
# 定义实际观测列的列表 (注意: 循环逻辑假设这里只有一个元素 'EOS')
acture3 = ['EOS']
...
for batch in range(total_batches):
    ...
    fig.text(0.04, 0.5, 'MOD43A4 EOS (DOY)', va='center', rotation='vertical', fontsize=30, weight='bold') # 调整 x, y 和 fontsize
    ...
    # 添加大图的X轴坐标（固定位置）
    # 添加大图的X轴坐标（固定位置）
    x_ticks_positions = [0.7, 0.65, 0.6, 0.55, 0.5, 0.45]
    for idx, pos in enumerate(x_ticks_positions):
        fig.text(0.85 - (idx * 0.135), 0.9, f"{pos:.0%}", ha='center', fontsize=24, weight='bold')

    # --- 保存图像 ---
    output_filename = f'F:\\pythonforR\\EoS_yanzhen4\\EOS_scatter_plots_withoutsjm_batch1_500_{batch + 1}.png'
