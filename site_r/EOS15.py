...
max_r_per_prefix = {
    prefix: {'r': -float('inf'), 'stage': None, 'threshold': None, 'data': None, 'slope': None, 'intercept': None,
             'rmse': float('inf'), 'method': None, 'threshold_pct': None} for prefix in
...
for batch in range(total_batches):
    ...
    # --- 保存图像 ---
    output_filename = f'F:\\pythonforR\\EoS_yanzhen4\\EOS_scatter_plots_withoutsjm_batch3_{batch + 1}.png'
...
# 添加大图标签
fig.text(0.5, 0.02, 'Leaf colouring (DOY)', ha='center', fontsize=30, weight='bold')
fig.text(0.02, 0.5, 'MOD43A4 EOS (DOY)', va='center', rotation='vertical', fontsize=30, weight='bold')
fig.suptitle('Best R Value for Each Threshold Type', fontsize=30, weight='bold', y=0.95)

# 保存汇总图
summary_output_filename = f'F:\\pythonforR\\EoS_yanzhen4\\EOS_best_R_summary.png'
