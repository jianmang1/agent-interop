...
def plot_nature_format(phases, matrices):
    ...
    # 设置坐标轴标签（加粗+中文）
    x_labels = phases  # 直接使用中文标签
    y_labels = phases  # 直接使用中文标签
    ax.set_xticks(np.arange(len(phases)))
    ax.set_xticklabels(x_labels, fontdict={'size': 7, 'weight': 'bold'})
    ax.set_yticks(np.arange(len(phases)))
    ax.set_yticklabels(y_labels, fontdict={'size': 7, 'weight': 'bold'})
    ...
    # 添加主标题（符合 Nature 风格）
    ax.set_title('GL Phenology and Climate Effects Matrix', fontsize=12, fontweight='bold', pad=20)

    # 图例（Nature 风格：无边框）
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='seagreen', lw=0, label='生长固有节律'),
        plt.Line2D([0], [0], marker='o', color='indianred', lw=0, label='温度效应'),
        plt.Line2D([0], [0], marker='o', color='steelblue', lw=0, label='降水效应')
    ]
...
if __name__ == "__main__":
    # 数据加载与处理
    df = load_data('F:/站点/中国地面/clime/clim_carryover_effect_gl.csv')
    ...
    fig.savefig('gl_phenology_matrix.png', dpi=300, bbox_inches='tight')
    plt.show()
