...
def main():
    # 1. 加载数据
    temp_df = load_climate_data('F:/站点/植物物候观测数据集20161202/CLIME/temp.csv')
    precip_df = load_climate_data('F:/站点/植物物候观测数据集20161202/CLIME/Preci.csv')

    phase_columns = ['Budburst','Flower','Fruit','Coloration']

    pheno_df = load_phenology_data('F:/站点/植物物候观测数据集20161202/CLIME/gl.csv', phase_columns)
    ...
    result_df.to_csv('F:\站点\植物物候观测数据集20161202\CLIME\clim_carryover_effect_gl.csv', index=False, encoding='gbk')
