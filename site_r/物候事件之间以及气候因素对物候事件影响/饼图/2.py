...
def export_matrices_to_csv(phases, matrices, filename_prefix="phenology_matrices"):
    """将矩阵数据导出到CSV文件"""
    data_rows = []
...
def export_matrices_as_excel(phases, matrices, filename="phenology_matrices_detailed.xlsx"):
...
if __name__ == "__main__":
    ...
    # 导出数据到CSV文件
    export_df = export_matrices_to_csv(valid_phases, matrices, "phenology_effects_export")

    # 导出详细数据到Excel文件
    export_matrices_as_excel(valid_phases, matrices, "E:\Desktop\灌草物候\phenology_effects_detailed.xlsx")
    ...
    fig.savefig('gl_aggregated_phenology_matrix.png', dpi=600, bbox_inches='tight')
    plt.show()
