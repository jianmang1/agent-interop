...
def load_data(filepath):
    df = pd.read_csv(
        ...
        usecols=['phase_early', 'phase_late', 'partial_corr_ph',
                 ...
                 'partial_corr_doy_temp', 'partial_corr_doy_precip',
                 'partial_corr_temp_precip', 'partial_corr_doy_temp_precip']
...
def aggregate_data(df):
    ...
    grouped = df.groupby(['phase_early', 'phase_late'], as_index=False).agg({
        'partial_corr_ph': lambda x: np.abs(np.nanmean(x)),
        'partial_corr_temp': lambda x: np.abs(np.nanmean(x)),
        'partial_corr_precip': lambda x: np.abs(np.nanmean(x)),
        'partial_corr_doy_temp': lambda x: np.abs(np.nanmean(x)),
        'partial_corr_doy_precip': lambda x: np.abs(np.nanmean(x)),
        'partial_corr_temp_precip': lambda x: np.abs(np.nanmean(x)),
        'partial_corr_doy_temp_precip': lambda x: np.abs(np.nanmean(x))
    })
    return grouped
...
def plot_normalized_pie_charts(phases, grouped_df):
    ...
    for i in range(len(phases)):
        for j in range(len(phases)):
            ...
            ph = row.iloc[0]['partial_corr_ph']
            temp = row.iloc[0]['partial_corr_temp']
            precip = row.iloc[0]['partial_corr_precip']

            temp_precip = row.iloc[0]['partial_corr_temp_precip']
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], color='#ff7f0e', lw=4, label='交互项效应')
    ]
...
if __name__ == "__main__":
    # 数据加载与处理
    df = load_data('F:/站点/中国地面/clime/交互/clim_carryover_effect_gl.csv')
