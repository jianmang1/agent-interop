...
# 保存到Excel文件
output_excel = os.path.join(output_dir, "mean_variance_results_去除离群值_plant.xlsx")
...
if stat_details:
    ...
    detail_excel = os.path.join(output_dir, "pairwise_mann_whitney_details_plant.xlsx")
...
for idx, config_name in enumerate(unique_configs):
    ...
    if idx // 3 == 2:  # 最后一行 (索引6,7,8)
        ax.set_xticklabels(labels_display, rotation=45, ha='right', fontsize=16)  # 添加 fontsize 参数
    else:
        ax.set_xticklabels([])  # 隐藏非最后一行的X轴标签
    ...
    if config_name in ['SOS', 'POS', 'EOS']:
        ...
        ylabel = 'days'
    elif config_name in ['ROG', 'ROS']:  # 修正：单独处理ROG和ROS
...
output_png = os.path.join(output_dir, "mean_variance_violinplot_3x3_去除离群值_plant_带显著性标注.png")
