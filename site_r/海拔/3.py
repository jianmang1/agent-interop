...
# 读取csv文件
df = pd.read_csv(r'D:\海拔\1\los_data.csv')
# 删除包含NaN的行
df.dropna(subset=['DEM', 'LOS'], inplace=True)
...
sns.scatterplot(x='DEM', y='LOS', color='green', data=df, s=1, alpha=0.3)  # 设置点的大小为1

plt.plot(df['DEM'], y_pred, color='green', label='Fitted line')  # 拟合线
