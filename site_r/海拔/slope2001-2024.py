...
for dataset in datasets:
    ...
    for year in range(2001, 2025):
        ...
        # 确保数据包含必要的列
        if variable not in data.columns or 'DEM' not in data.columns:
            print(f"文件 {file_name} 缺少必要的列（{variable} 或 DEM），跳过。")
            continue

        # 清理数据：删除空值，并确保数据类型正确
        # 清理数据：删除空值、确保数据类型正确，并删除 data[variable] 值为 1 的行
        data = data.dropna(subset=[variable, 'DEM'])  # 删除 variable 和 DEM 列中的空值
        data[variable] = pd.to_numeric(data[variable], errors='coerce')  # 确保变量列是数值类型
        data['DEM'] = pd.to_numeric(data['DEM'], errors='coerce')  # 确保 DEM 列是数值类型
        data = data[(data[variable].notnull()) & (data['DEM'].notnull())]  # 删除转换后仍为空值的行
