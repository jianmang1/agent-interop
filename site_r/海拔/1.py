...
# 读取csv文件
df = pd.read_csv(r'D:\海拔\1\eos_data.csv')

# 删除包含NaN的行
df.dropna(subset=['DEM', 'EOS'], inplace=True)
...
sns.scatterplot(x='DEM', y='EOS', color='purple', data=df, s=1, alpha=0.3)  # 设置点的大小为1

plt.plot(df['DEM'], y_pred, color='purple', label='Fitted line')  # 拟合线
