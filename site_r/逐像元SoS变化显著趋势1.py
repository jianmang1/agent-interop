...
files_direct = [os.path.join(directory, f'masked2_masked_all_EoS_DOY_{year}.tif') for year in years if
                os.path.exists(os.path.join(directory, f'masked2_masked_all_EoS_DOY_{year}.tif'))]
...
for window in windows:
    ...
    mean_values = np.where(np.isnan(mean_values), -9999, mean_values).astype(np.float32)
    rmse_values = np.where(np.isnan(rmse_values), -9999, rmse_values).astype(np.float32)
    slope_values = np.where(np.isnan(slope_values), -9999, slope_values).astype(np.float32)
    p_values = np.where(np.isnan(p_values), -9999, p_values).astype(np.float32)
