...
# 读取CSV数据
df = pd.read_csv('F:\\pythonforR\\sos3\\sos_withoutsjm.csv')
...
threshold_prefixes = ['SoS_doublelogisticthreshold_', 'mintomaxdoublelogistic_SoS_threshold_',
                      'SoS_filterdoublelogisticthreshold_', 'SoS_filterthreshold_',
                      'SoS_mintomaxthreshold_', 'cubic_SoS_threshold_', 'filtercubic_SoS_threshold_',
...
# 构建所有可能的阈值列名并筛选出存在的列
all_threshold_columns1 = [f"{prefix}{method}{i}" for prefix in threshold_prefixes for method in meth for i in
                          [1, 13, 2, 3, 4, 5]]
...
for batch in range(total_batches):
    ...
    # 添加大图的大标题
    fig.text(0.5, 0.04, 'Bud burst (DOY)', ha='center', fontsize=30, weight='bold')
    ...
    # 添加大图的X轴坐标（固定位置）
    # 添加大图的X轴坐标（固定位置）
    x_ticks_positions = [0.5,0.4,0.3,0.2,0.15,0.1]
    for idx, pos in enumerate(x_ticks_positions):
        fig.text(0.85 - (idx * 0.135), 0.07, str(pos), ha='center', fontsize=24, weight='bold')
    # 保存当前批次的大图到文件
    plt.savefig(f'D:\\GEE(PYTHON)\\YANZHEN\\sos\\SOS_scatter_plots_withoutsjm_batch2_{batch + 1}.png', dpi=600, bbox_inches='tight')
    plt.clf()
    plt.close(fig)