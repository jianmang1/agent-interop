...
# 定义掩膜文件路径
mask_files = [
    r"D:\MOD12Q2\vs\classai\plant\1.tif",
    r"D:\MOD12Q2\vs\classai\plant\2.tif",
    r"D:\MOD12Q2\vs\classai\plant\3.tif",
    r"D:\MOD12Q2\vs\classai\plant\4.tif"
]
...
mean_config = [
    ...
    (r'D:\MODIS43A4_2024\masked_without_tibet\ROG\slope_values.tif', 'ROG slope'),  # 修正路径
    (r'D:\MODIS43A4_2024\masked_without_tibet\ROS\slope_values.tif', 'ROS slope'),  # 修正路径
]
...
# 保存到Excel文件
output_excel = os.path.join(output_dir, "mean_variance_results_去除离群值_plant_trend.xlsx")
...
if stat_details:
    ...
    detail_excel = os.path.join(output_dir, "pairwise_mann_whitney_details_plant_trend.xlsx")
...
display_names = ['Cropland', 'Forest', 'Shrub', 'Grassland', 'All']
colors = ['#FFB579', '#6BBBA1', '#A8D5BA', '#E4EBB0', '#AAAAAA']
...
for idx, config_name in enumerate(unique_configs):
    ...
    # 添加均值数值标注（仅在均值线上显示，白色艺术字，最上层，线中间）
    for i, data in enumerate(data_list):
        ...
        if 'cmeans' in parts:
            ...
            if i < len(segments):
                ...
                # 添加白色艺术字标注（最上层，线中间）
                ax.text(x_mean, y_mean, f'{mean_val:.4f}',
                        ha='left', va='bottom',
    ...
    if config_name in ['SOS slope', 'POS slope', 'EOS slope', 'LOS slope']:
        ...
        ylabel = 'VI day$^{-1}$ year$^{-1}$'  # 使用上标表示 -1
    else:
        ylabel = 'Value'
    ax.set_ylabel(ylabel, fontsize=20)

    ax.set_title(config_name, fontsize=20, pad=8)
    ...
    if len(all_data_values) > 0:
        ...
        y_range = y_max - y_min
        padding = y_range * 0.15  # 添加更多边距给字母标注
...
output_png = os.path.join(output_dir, "mean_variance_violinplot_3x3_去除离群值_plant_trend_带显著性标注.png")
