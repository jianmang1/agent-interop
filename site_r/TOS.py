...
# 定义路径和年份范围
sos_dir = r'D:\MODIS43A4_Value\AOS'
eos_dir = r'D:\MODIS43A4_Value\POS'
years = range(2001, 2025)

# 构建文件路径列表
sos_files = [os.path.join(sos_dir, f'masked2_masked_all_AoS_value_{year}.tif') for year in years]
eos_files = [os.path.join(eos_dir, f'masked2_masked_all_PoS_value_{year}.tif') for year in years]

# 创建输出目录
output_dir = r'D:\MODIS43A4_Value\TOS'
...
for year, (sos_file, eos_file) in zip(years, zip(sos_files, eos_files)):
    try:
        ...
        with rasterio.open(sos_file) as sos_src, rasterio.open(eos_file) as eos_src:
            ...
            # 保存结果到新的 TIFF 文件
            output_file = os.path.join(output_dir, f'TOS_{year}.tif')
