...
# 定义文件路径
directory = r'F:\MODIS43A4_DOY\GOSbeLOS'
years = list(range(2001, 2024))  # 年份列表
# 创建一个文件路径列表，匹配文件名模式为 all_SoS_value_<year>_masked.tif
files = [os.path.join(directory, f) for f in os.listdir(directory) if
         any(f == f'result_{year}.tif' for year in years)]
files_direct = [os.path.join(directory, f'result_{year}.tif') for year in years if os.path.exists(os.path.join(directory, f'result_{year}.tif'))]
