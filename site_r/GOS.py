...
# 定义路径和年份范围
sos_dir = r'D:\MODIS43A4_2024\SOS\500m(unmask0)'
eos_dir = r'D:\MODIS43A4_2024\POS\unmask0'
years = range(2001, 2025)

# 构建文件路径列表
sos_files = [os.path.join(sos_dir, f'masked2_masked_all_SoS_DOY_{year}.tif') for year in years]
eos_files = [os.path.join(eos_dir, f'masked2_masked_all_PoS_DOY_{year}.tif') for year in years]

# 创建输出目录
output_dir = r'D:\MODIS43A4_2024\GOS'
...
for year, (sos_file, eos_file) in zip(years, zip(sos_files, eos_files)):
    try:
        ...
        with rasterio.open(sos_file) as sos_src, rasterio.open(eos_file) as eos_src:
            ...
            # 保存结果到新的 TIFF 文件
            output_file = os.path.join(output_dir, f'GOS_{year}.tif')
            with rasterio.open(output_file, 'w', **out_meta) as dst:
                dst.write(los_data, 1)

        print(f"成功生成 {year} 年的 GOS 文件: {output_file}")
