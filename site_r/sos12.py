...
# 读取CSV数据
df = pd.read_csv('F:\\pythonforR\\sos3\\sossite_500.csv')
...
for batch in range(total_batches):
    ...
    # 保存当前批次的大图到文件
    plt.savefig(f'F:\\pythonforR\\sos3\\SOS_scatter_plots_withoutsjm_batch1_{batch + 1}.png', dpi=600, bbox_inches='tight')
    plt.clf()
    plt.close(fig)