...
# 初始化存储结果的字典
results = {value: {'means': [], 'stds': [], 'values': [], 'pixel_counts': []} for value in values_to_extract}
...
for grassland_file in grassland_files:
    ...
    # 重采样
    grassland_data = resample_raster(grassland_file, temp_file, land_x_size, land_y_size, land_geo_transform,
                                     land_projection)
    ...
    for value in values_to_extract:
        ...
        if len(grassland_values) > 0:
            # 将无效像元值（255）视为0
            grassland_values = np.where(grassland_values == 255, 0, grassland_values)

            # 过滤掉 NaN 和无穷大值
            grassland_values = np.array(grassland_values)
            grassland_values = grassland_values[np.isfinite(grassland_values)]

            if len(grassland_values) > 0:
                # 计算平均值和标准差
                mean_grassland = np.mean(grassland_values)
                std_grassland = np.std(grassland_values)

                # 存储结果
                results[value]['means'].append(mean_grassland)
                results[value]['stds'].append(std_grassland)
                results[value]['values'].extend(grassland_values)
                results[value]['pixel_counts'].append(len(grassland_values))

# 输出到Excel
excel_file = 'F:\\个人\\grassland_cover_results1111.xlsx'
