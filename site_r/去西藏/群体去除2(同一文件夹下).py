    ...
    with rasterio.open(input_raster_path) as src:
        ...
        if len(processed_data.shape) == 3:  # 多波段
            for i in range(processed_data.shape[0]):
                processed_data[i, tibet_mask] = -9999
        else:  # 单波段
            processed_data[tibet_mask] = -9999
...
def main():
    # 定义输入目录和输出目录
    input_directory = r"E:\Desktop\mai\LOS"
    tibet_boundary = r"D:\MODIS43A4_2024\西藏\西藏自治区_省.geojson"

    # 生成要处理的文件列表 (2001-2024年)
    files_to_process = []
    for year in range(2001,2025):
        filename = f'masked2_masked_all_AoS_value_{year}.tif'
