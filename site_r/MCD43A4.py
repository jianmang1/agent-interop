...
# 第二个子图
df2 = pd.read_csv(r'F:\pythonforR\POS\pos_nosjm3.csv')
valid_data2 = df2[['fruit', 'POS_cublicNDPI']]
stats2 = calculate_stats(valid_data2, 'fruit', 'POS_cublicNDPI')
plot_subplot(axs[1], df2, 'fruit', 'POS_cublicNDPI', 'POS', stats2, legend_title='fruit (DOY)', show_site_legend=True)
...
plot_subplot(axs[2], df3, 'EOS', 'EoS_filterdoublelogistic_threshold_EVI2_7', 'EOS', stats3, legend_title=None, show_site_legend=False)
fig.text(0.85, 0.29, 'Leaf colouring (DOY)', fontsize=8, horizontalalignment='right')  # 调整x坐标值以匹配子图的位置
axs[2].set_xlim(200, 350)
axs[2].set_ylim(200, 350)
axs[2].set_xticks(np.arange(200, 351, 50))
axs[2].set_yticks(np.arange(200, 351, 50))
# 设置大图的x轴和y轴标题
