...
# --- 配置 ---
# 读取CSV数据
df = pd.read_csv('F:\\pythonforR\\EoS_yanzhen4\\1.csv')

if 'AI' not in df.columns:
    raise ValueError("数据中缺少 'AI' 列！请确认 CSV 文件包含 AI 列。")

# 定义实际观测列的列表
acture3 = ['EOS']

# 定义阈值列的前缀列表
threshold_prefixes = ['doublelogistic_threshold_', 'filterdoublelogistic_threshold_',
                      'mintomaxdoublelogistic_threshold_',
                      'cubic_threshold_', 'filtercubic_threshold_', 'mintomaxcubic_threshold_',
                      'threshold_', 'filterthreshold_', 'mintomaxthreshold_', ]

# 定义方法列表 - 按长度降序排列，确保较长的方法名优先匹配
meth = ['NDVI', 'EVI2', 'EVI', 'NDPI', 'NDGI']

# 构建所有可能的阈值列名并筛选出存在的列
all_threshold_columns = [f"EoS_{prefix}{method}_{i}" for prefix in threshold_prefixes for method in meth for i in
                         [45, 5, 55, 6, 65, 7]]
...
fig_width_inch = 1000 / 25.4  # 宽度 580mm 转换为英寸
fig_height_inch = 6 * 3.5  # 假设每行一个方法，共6行(NDVI,EVI,EVI2,NDPI,NDGI + 标题/标签空间)，每行4英寸
letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i']  # 为9个小图准备字母

# 定义阈值百分比标签
x_ticks_positions = [0.45, 0.5, 0.55, 0.6, 0.65, 0.7]

# 按 AI 值排序站点
df['AI'] = pd.to_numeric(df['AI'], errors='coerce')
site_ai_map = df[['Site', 'AI']].drop_duplicates().dropna()
site_ai_sorted = site_ai_map.sort_values('AI')
unique_sites_sorted = site_ai_sorted['Site'].tolist()

# 创建基于 AI 排序的颜色映射（颜色反转）
if unique_sites_sorted:
    # 使用反转的 viridis 调色板 (viridis_r)
    site_palette = sns.color_palette('viridis_r', n_colors=len(unique_sites_sorted))
    site_color_map = dict(zip(unique_sites_sorted, site_palette))
else:
    unique_sites_sorted = []
    site_color_map = {}
...
max_r_per_prefix = {
    prefix: {'r': -float('inf'), 'stage': None, 'threshold': None, 'data': None, 'slope': None, 'intercept': None,
             'rmse': float('inf'), 'method': None, 'threshold_pct': None, 'p_value': float('nan')} for prefix in
    threshold_prefixes}  # 初始化rmse为无穷大
...
for batch in range(total_batches):
    ...
    fig, axes = plt.subplots(nrows=len(meth), ncols=6, figsize=(fig_width_inch, fig_height_inch),
                             constrained_layout=False, gridspec_kw={'wspace': 0.8, 'hspace': 0.4})  # 增大wspace
    ...
    for global_plot_index in range(start_index, end_index):
        ...
        # --- 数据清洗关键步骤 ---
        # 1. 移除原始数据中的 NaN
        valid_data_raw = df.dropna(subset=[stage, threshold, 'Site', 'AI'])
        ...
        valid_data_numeric['AI'] = pd.to_numeric(valid_data_numeric['AI'], errors='coerce')

        # 3. 再次移除因转换产生的 NaN
        valid_data_final = valid_data_numeric.dropna(subset=[stage, threshold, 'Site', 'AI'])
        ...
        try:
            ...
            # 更精确的解析方式 - 避免子串匹配错误
            for method in meth:  # 按预定义顺序匹配
                # 使用正则表达式或更精确的模式匹配
                if f"_{method}_" in threshold:
                    extracted_method = method
                    break

            # 如果没有找到匹配的方法，尝试查找方法名在列名中的确切位置
            if extracted_method is None:
                for method in meth:
                    # 检查方法名是否作为独立部分出现在列名中
                    parts = threshold.split('_')
                    for part in parts:
                        if part == method:  # 确保完全匹配
                            extracted_method = method
                            break
                    if extracted_method:
                        break
            ...
            # 提取阈值百分比数字
            # 查找阈值数字：45, 50(5), 55, 60(6), 65, 70(7)
            number_map = {5: 50, 6: 60, 7: 70}  # 映射简写到完整百分比
            for num in [45, 5, 55, 6, 65, 7]:
                # 在列名的最后部分查找数字
                parts = threshold.split('_')
                # 检查最后几个部分是否包含数字
            ...
            # 检查是否是当前前缀的最大R值或相同R值但更小RMSE（保留两位小数进行比较）
            for prefix in threshold_prefixes:
                if threshold.startswith(f"EoS_{prefix}"):
                    current_r = r
                    current_rmse = rmse
                    current_p_value = p_value
                    stored_r = max_r_per_prefix[prefix]['r']
                    stored_rmse = max_r_per_prefix[prefix]['rmse']
                    ...
                    if rounded_current_r > rounded_stored_r:
                        should_update = True
                    elif rounded_current_r == rounded_stored_r and current_rmse < stored_rmse:
                        should_update = True
                    ...
                    if should_update:
                        ...
                        max_r_per_prefix[prefix]['p_value'] = current_p_value
                    break  # 找到对应的前缀后跳出内层循环

            # --- 绘图 ---
            ax = axes_flat[plot_counter_in_batch]

            # 绘制散点图 (根据Site列使用不同颜色，按AI值排序)
            if 'Site' in valid_data_final.columns and valid_data_final['Site'].notna().any():
                # 按站点颜色映射
                for site in valid_data_final['Site'].unique():
                    site_data = valid_data_final[valid_data_final['Site'] == site]
                    color = site_color_map.get(site, 'black')  # 如果站点没有映射则使用黑色
                    sns.scatterplot(
                        x=site_data[stage],
                        y=site_data[threshold],
                        ax=ax,
                        color=color,
                        alpha=0.7,
                        s=20,
                ...
                # 如果没有 Site 信息，回退到黑色点
                sns.scatterplot(
                    x=valid_data_final[stage],
                    y=valid_data_final[threshold],
                    ax=ax,
                    color='black',
                    alpha=0.6,
                    s=20
    ...
    # --- 添加大图标签 (调整字体大小和位置) ---
    fig.text(0.5, 0.06, 'Leaf colouring (DOY)', ha='center', fontsize=30, weight='bold')  # 调整 y 和 fontsize
    fig.text(0.04, 0.5, 'MOD43A4 EOS (DOY)', va='center', rotation='vertical', fontsize=30,
    ...
    # --- 保存图像 ---
    output_filename = f'F:\\pythonforR\\EoS_yanzhen4\\EOS_scatter_plots_withoutsjm_batch3_{prefix}.png'
...
for i, prefix in enumerate(threshold_prefixes):
    ...
    if max_r_per_prefix[prefix]['r'] == -float('inf'):
        ...
        best_p_value = max_r_per_prefix[prefix]['p_value']

        # 绘制散点图 (根据Site列使用不同颜色，按AI值排序)
        if 'Site' in best_data.columns and best_data['Site'].notna().any():
            # 按站点颜色映射
            for site in best_data['Site'].unique():
                site_data = best_data[best_data['Site'] == site]
                color = site_color_map.get(site, 'black')  # 如果站点没有映射则使用黑色
                sns.scatterplot(
                    x=site_data[best_stage],
                    y=site_data[best_threshold],
                    ax=ax,
                    color=color,
                    alpha=0.7,
                    s=20,
            ...
            sns.scatterplot(
                x=best_data[best_stage],
                y=best_data[best_threshold],
                ax=ax,
                color='black',
                alpha=0.6,
                s=20
        ...
        ax.text(0.95, 0.2, f'R: {best_r:.2f}',
                transform=ax.transAxes,
                horizontalalignment='right',
                verticalalignment='bottom',
                fontsize=24,
        ...
        ax.text(0.95, 0.12, f'RMSE: {best_rmse:.2f}',
                transform=ax.transAxes,
                horizontalalignment='right',
                verticalalignment='bottom',
                fontsize=24,
        ...
        # 添加P值信息（右下角）
        p_text = f'P: {best_p_value:.3f}' if not np.isnan(best_p_value) else 'P: N/A'
        ax.text(0.95, 0.04, p_text,
                transform=ax.transAxes,
                horizontalalignment='right',
                verticalalignment='bottom',
                fontsize=24,
        ...
        # 添加子图标签 - 包含序号 (a)EVI 30%
        if best_method and best_threshold_pct is not None:
            # 确保显示的是完整的百分比，如70%而不是7%
            label = f'({letters[i]}) {best_method} {best_threshold_pct * 100:.0f}%'
        else:
            # 如果无法解析method和pct，则使用前缀名
            label = f'({letters[i]}) {prefix.replace("_threshold_", "")}'
...
if unique_sites_sorted:
    ...
    legend_elements = [plt.Line2D([0], [0], marker='o', color='w',
                                  markerfacecolor=site_color_map[site],
                                  markersize=10, label=site)
                       for site in unique_sites_sorted]

    # 添加图例到底部
    fig.legend(handles=legend_elements, loc='lower center', bbox_to_anchor=(0.5, 0.01),
               title='Sites (sorted by AI)', fontsize=20, title_fontsize=24, ncol=min(len(unique_sites_sorted), 10))

# 保存汇总图
summary_output_filename = f'F:\\pythonforR\\EoS_yanzhen4\\EOS_best_R_summary2.png'
